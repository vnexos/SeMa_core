...
/**
 * Provides engine for Oracle database
 * 
 * @author Trần Việt Đăng Quang
 * @see DatabaseEngine
 */
public class OracleEngine implements DatabaseEngine {
  ...
  private enum ColumnType {
    ...
    DATE("DATE"),
    TIMESTAMP("TIMESTAMP"),
    BOOLEAN("NUMBER(1)"), // Oracle does not have a native BOOLEAN type
    UUID("RAW(16)"); // Oracle uses RAW(16) for UUIDs
  ...
  private static final Map<Class<?>, TypeMapping> TYPE_MAPPINGS = Map.ofEntries(
      ...
      Map.entry(Boolean.class, new TypeMapping(ColumnType.BOOLEAN, 0)),
      Map.entry(byte.class, new TypeMapping(ColumnType.NUMBER, 3)),
      Map.entry(Byte.class, new TypeMapping(ColumnType.NUMBER, 3)),
      Map.entry(short.class, new TypeMapping(ColumnType.NUMBER, 5)),
      Map.entry(Short.class, new TypeMapping(ColumnType.NUMBER, 5)),
      Map.entry(int.class, new TypeMapping(ColumnType.NUMBER, 10)),
      Map.entry(Integer.class, new TypeMapping(ColumnType.NUMBER, 10)),
      Map.entry(long.class, new TypeMapping(ColumnType.NUMBER, 19)),
      Map.entry(Long.class, new TypeMapping(ColumnType.NUMBER, 19)),
      Map.entry(java.math.BigInteger.class, new TypeMapping(ColumnType.NUMBER, 38)),
      Map.entry(float.class, new TypeMapping(ColumnType.BINARY_FLOAT, 0)),
      Map.entry(Float.class, new TypeMapping(ColumnType.BINARY_FLOAT, 0)),
      Map.entry(double.class, new TypeMapping(ColumnType.BINARY_DOUBLE, 0)),
      Map.entry(Double.class, new TypeMapping(ColumnType.BINARY_DOUBLE, 0)),
      Map.entry(char.class, new TypeMapping(ColumnType.CHAR, 1)),
      Map.entry(Character.class, new TypeMapping(ColumnType.CHAR, 1)),
      Map.entry(String.class, new TypeMapping(ColumnType.VARCHAR2, 255)),
      ...
      Map.entry(java.sql.Time.class, new TypeMapping(ColumnType.TIMESTAMP, 0)),
      Map.entry(java.time.LocalTime.class, new TypeMapping(ColumnType.TIMESTAMP, 0)),
      Map.entry(java.sql.Timestamp.class, new TypeMapping(ColumnType.TIMESTAMP, 0)),
      Map.entry(java.time.LocalDateTime.class, new TypeMapping(ColumnType.TIMESTAMP, 0)),
      Map.entry(java.time.Instant.class, new TypeMapping(ColumnType.TIMESTAMP, 0)),
      Map.entry(byte[].class, new TypeMapping(ColumnType.BLOB, 0)),
      Map.entry(Byte[].class, new TypeMapping(ColumnType.BLOB, 0)),
      Map.entry(java.math.BigDecimal.class, new TypeMapping(ColumnType.NUMBER, 0)));
  ...
  private String getCustomColumnType(String customType, int length) {
    switch (customType) {
      case "NUMBER":
        return ColumnType.NUMBER.getType(length);
      case "FLOAT":
        return ColumnType.FLOAT.getType(length);
      case "BINARY_FLOAT":
        return ColumnType.BINARY_FLOAT.getType(length);
      case "BINARY_DOUBLE":
        return ColumnType.BINARY_DOUBLE.getType(length);
      case "CHAR":
        return ColumnType.CHAR.getType(length);
      case "VARCHAR2":
        return ColumnType.VARCHAR2.getType(length);
      case "CLOB":
        return ColumnType.CLOB.getType(length);
      case "RAW":
        return ColumnType.RAW.getType(length);
      case "BLOB":
  ...
  private String getTypeBasedColumnType(Class<?> type) {
    if (type.isEnum()) {
      return ColumnType.VARCHAR2.getType(255);
    }
  ...
  public String generateCreateDatabase(String databaseName) throws SQLException {
    // Oracle does not use CREATE DATABASE; schemas are created with users
    return "CREATE USER " + getNameOpening() + databaseName + getNameEnding() + " IDENTIFIED BY password";
  }
    ...
    for (String key : object.keySet()) {
      ...
      if (element.isJsonNull()) {
        ...
        if (element.getAsJsonPrimitive().isString()) {
          ...
          values.append(element.getAsBoolean() ? "1" : "0"); // Oracle uses NUMBER(1) for BOOLEAN
        } else {
    ...
    for (String key : jsonObject.keySet()) {
      ...
      if (element.isJsonNull()) {
        ...
        if (element.getAsJsonPrimitive().isString()) {
          ...
          setClause.append(element.getAsBoolean() ? "1" : "0"); // Oracle uses NUMBER(1) for BOOLEAN
        } else {
  ...
  private String generateFieldSql(Field field) throws SQLException {
    ...
    if (identity != null && identity.type() == IdType.PRIMITIVE && ClassUtils.isPrimitive(field.getType())) {
      // Oracle uses sequences and triggers for identity columns
      String sequenceName = StringUtils.convertCamelToSnake(field.getName()) + "_seq";
      sb.append(" DEFAULT " + sequenceName + ".NEXTVAL");
    }
