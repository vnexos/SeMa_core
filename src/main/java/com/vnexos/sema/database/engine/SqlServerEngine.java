...
/**
 * Provides engine for Microsoft SQL Server database
 * 
...
public class SqlServerEngine implements DatabaseEngine {
  ...
  private static final Map<Class<?>, TypeMapping> TYPE_MAPPINGS = Map.ofEntries(
      Map.entry(boolean.class, new TypeMapping(ColumnType.BIT, 0)),
      Map.entry(Boolean.class, new TypeMapping(ColumnType.BIT, 0)),
      Map.entry(byte.class, new TypeMapping(ColumnType.TINYINT, 0)),
      Map.entry(Byte.class, new TypeMapping(ColumnType.TINYINT, 0)),
      Map.entry(short.class, new TypeMapping(ColumnType.SMALLINT, 0)),
      Map.entry(Short.class, new TypeMapping(ColumnType.SMALLINT, 0)),
      Map.entry(int.class, new TypeMapping(ColumnType.INT, 0)),
      Map.entry(Integer.class, new TypeMapping(ColumnType.INT, 0)),
      ...
      Map.entry(float.class, new TypeMapping(ColumnType.REAL, 0)),
      Map.entry(Float.class, new TypeMapping(ColumnType.REAL, 0)),
      Map.entry(double.class, new TypeMapping(ColumnType.FLOAT, 0)),
      Map.entry(Double.class, new TypeMapping(ColumnType.FLOAT, 0)),
      ...
      Map.entry(UUID.class, new TypeMapping(ColumnType.UNIQUEIDENTIFIER, 0)),
      ...
      Map.entry(byte[].class, new TypeMapping(ColumnType.VARBINARY, 8000)),
      Map.entry(Byte[].class, new TypeMapping(ColumnType.VARBINARY, 8000)),
  ...
  private String getCustomColumnType(String customType, int length) {
    switch (customType) {
      ...
      case "INT":
      case "INTEGER":
        return ColumnType.INT.getType(length);
      case "BIGINT":
      ...
      case "REAL":
        return ColumnType.REAL.getType(length);
      case "CHAR":
      ...
      case "VARBINARY":
        return ColumnType.VARBINARY.getType(length);
      case "IMAGE":
        return ColumnType.IMAGE.getType(length);
      case "DATE":
      ...
      case "BIT":
        return ColumnType.BIT.getType(length);
      case "UNIQUEIDENTIFIER":
        return ColumnType.UNIQUEIDENTIFIER.getType(length);
      default:
        return customType;
    }
  }
  ...
  public String generateCreateDatabase(String databaseName) throws SQLException {
    return "CREATE DATABASE " + getNameOpening() + databaseName + getNameEnding();
  }
    ...
    for (String key : object.keySet()) {
      ...
      if (element.isJsonNull()) {
        ...
        if (element.getAsJsonPrimitive().isString()) {
          ...
          values.append(element.getAsBoolean() ? "1" : "0"); // SQL Server uses BIT for BOOLEAN
        } else {
    ...
    for (String key : jsonObject.keySet()) {
      ...
      if (element.isJsonNull()) {
        ...
        if (element.getAsJsonPrimitive().isString()) {
          ...
          setClause.append(element.getAsBoolean() ? "1" : "0"); // SQL Server uses BIT for BOOLEAN
        } else {
