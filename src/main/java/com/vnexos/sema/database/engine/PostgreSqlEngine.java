...
/**
 * Provides engine for PostgreSQL database
 * 
...
public class PostgreSqlEngine implements DatabaseEngine {
  ...
  private static final Map<Class<?>, TypeMapping> TYPE_MAPPINGS = Map.ofEntries(
      ...
      Map.entry(Boolean.class, new TypeMapping(ColumnType.BOOLEAN, 0)),
      Map.entry(byte.class, new TypeMapping(ColumnType.SMALLINT, 0)),
      Map.entry(Byte.class, new TypeMapping(ColumnType.SMALLINT, 0)),
      ...
      Map.entry(byte[].class, new TypeMapping(ColumnType.BYTEA, 0)),
      Map.entry(Byte[].class, new TypeMapping(ColumnType.BYTEA, 0)),
      Map.entry(java.math.BigDecimal.class, new TypeMapping(ColumnType.NUMERIC, 0)));
  ...
  private String getCustomColumnType(String customType, int length) {
    switch (customType) {
      ...
      case "NUMERIC":
      case "DECIMAL":
        return ColumnType.NUMERIC.getType(length);
      case "REAL":
      ...
      case "BYTEA":
        return ColumnType.BYTEA.getType(length);
      case "DATE":
      ...
      case "JSON":
        return ColumnType.JSON.getType(length);
      case "JSONB":
        return ColumnType.JSONB.getType(length);
      default:
        return customType;
    }
  }
